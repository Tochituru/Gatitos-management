...
const createCell = (fieldClass, fieldValue) => {
	...
	if (fieldClass === 'date') {
		...
		// esta transformacion del string se podria hacer directo sobre la linea anterior.
		// also, moment.js es la mejor opción.
...
const createTable = (element) => {
	element.forEach(({ id, name, date, color, toy, email }) => {
		...
		newRow.appendChild(createCell('name', name))
		newRow.appendChild(createCell('date', date))
		...
		innerAction.appendChild(
			createBtn('editBtn', id, editImg, "toggleModal('editModal', 'editBtn'),  getKittenId()")
		)
...
const initialize = () => {
	fetch(api).then((res) => res.json()).then(({ kittens }) => {
...
//hacer el post
const cleanForm = (formToClean) =>
	formToClean.forEach((inputElement) => {
		inputElement.value = ''
...
const getKittenId = () => {
	...
	fetch(`${api}/id/${id}`).then((res) => res.json()).then(({ id, name, date, color, toy, email }) => {
		// guarda con ese uso de indices, puede generar efectos inesperados
...
const filterKittens = () => {
	...
	fetch(`${api}/search/?name=${searchField}`).then((res) => res.json()).then((kittens) => {
		kittenTable.innerHTML = ''
		createTable(kittens)
	})
}
